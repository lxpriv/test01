...
if build_dir == "/home/aistudio":
    build_dir += "/build"
    with open("/home/aistudio/build/trained_models/model_list.txt", 'r') as file:
        for line in file:
            os.system(f"cp -nr /home/aistudio/data/data230946/{line.strip()} /home/aistudio/build/trained_models/")
model_dir=build_dir+'/trained_models'

model_list_path = model_dir + "/model_list.txt"
with open(model_list_path, 'r') as file:
    for line in file:
        if os.path.exists(model_dir+"/"+line.strip()) == False:
            utils.download_file(f"https://ysu-svc.oss-rg-china-mainland.aliyuncs.com/{line.strip()}", model_dir+"/"+line.strip())
...
with app:
    ...
    with gr.Tabs() as tabs:
        ...
        with gr.TabItem("转换"):
            model_name = gr.Dropdown(label="模型", choices=models, value="派蒙")
